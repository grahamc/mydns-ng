...
import_rr(char *name, char *type, char *data, unsigned aux, unsigned ttl)
{
	...
	esc_data = sql_escstr(sql, data);
	if (IXFR) {
	  querystr = "INSERT INTO %s (zone,name,type,data,aux,ttl,active,serial) VALUES (%u,'%s','%s','%s',%u,%u,'%s',%u);";
#if DEBUG_SQL
	  Debug(querystr, mydns_rr_table_name,
		import_zone_id, esc_name, type, esc_data, aux, ttl, ACTIVE, import_serial);
#endif
	  sql_queryf(sql,
		     querystr, mydns_rr_table_name,
		     import_zone_id, esc_name, type, esc_data, aux, ttl, ACTIVE, import_serial);
	} else if (ACTIVE) {
	  querystr = "INSERT INTO %s (zone,name,type,data,aux,ttl,active) VALUES (%u,'%s','%s','%s',%u,%u,'%s');";
#if DEBUG_SQL
	  Debug(querystr, mydns_rr_table_name,
		import_zone_id, esc_name, type, esc_data, aux, ttl, ACTIVE);
#endif
	  sql_queryf(sql,
		     querystr, mydns_rr_table_name,
		     import_zone_id, esc_name, type, esc_data, aux, ttl, ACTIVE);
	} else {
	  querystr = "INSERT INTO %s (zone,name,type,data,aux,ttl) VALUES (%u,'%s','%s','%s',%u,%u);";
#if DEBUG_SQL
	  Debug(querystr, mydns_rr_table_name,
