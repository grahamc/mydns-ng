...
void mydns_check_name(MYDNS_SOAP soa, MYDNS_RRP rr, const char *name, char *data,
  ...
  if (!strlen(fqdn))
    return mydns_rrproblem(soa, rr, name, data, _("FQDN in `%s' is empty"), col);

  if (strlen(fqdn) > DNS_MAXNAMELEN)
    return mydns_rrproblem(soa, rr, name, data, _("FQDN in `%s' is too long"), col);
  ...
  if (strcmp(fqdn, ".")) {
	...
	if (strlen(label))
	  mydns_rrproblem(soa, rr, name, data, _("Last label in `%s' not the root zone"), col);
	break;
      }
      if (strcmp(label, "*")) {
	if (len > DNS_MAXLABELLEN)
	  mydns_rrproblem(soa, rr, name, data, _("Label in `%s' is too long"), col);
	if (len < 1)
	  mydns_rrproblem(soa, rr, name, data, _("Blank label in `%s'"), col);
	for (cp = label; *cp; cp++) {
	  if (*cp == '-' && cp == label)
	    mydns_rrproblem(soa, rr, name, data, _("Label in `%s' begins with a hyphen"), col);
	  if (*cp == '-' && ((cp - label) == len-1))
	    mydns_rrproblem(soa, rr, name, data, _("Label in `%s' ends with a hyphen"), col);
	  if (!isalnum((int)(*cp)) && *cp != '-') {
	    if (is_rr && *cp == '*')
	      mydns_rrproblem(soa, rr, name, data, _("Wildcard character `%c' in `%s' not alone"), *cp, col);
	    else if (*cp == '_' && allow_underscore)
	      ;
	    else
	      mydns_rrproblem(soa, rr, name, data, _("Label in `%s' contains illegal character `%c'"), col, *cp);
	  }
	}
      } else if (!is_rr)
	mydns_rrproblem(soa, rr, name, data, _("Wildcard not allowed in `%s'"), col);
    }
    RELEASE(buf);
  }
...
void __mydns_check_rr_a(MYDNS_SOAP soa, MYDNS_RRP rr, const char *name, char *data,
  ...
  if (rr->alias == 1)
    ...
    if (inet_pton(AF_INET, a_in, (void *)&addr) <= 0)
      mydns_rrproblem(soa, rr, name, data, _("IPv4 address in `data' is invalid"));
  }
}
/*--- __mydns_check_rr_a() --------------------------------------------------------------------------*/
...
void __mydns_check_rr_aaaa(MYDNS_SOAP soa, MYDNS_RRP rr, const char *name, char *data,
  ...
  if (inet_pton(AF_INET6, aaaa_in, (void *)&addr) <= 0)
    mydns_rrproblem(soa, rr, name, data, _("IPv6 address in `data' is invalid"));
}
/*--- __mydns_check_rr_aaaa() --------------------------------------------------------------------------*/
...
void __mydns_check_rr_cname(MYDNS_SOAP soa, MYDNS_RRP rr, const char *name, char *data,
  ...
  data = mydns_expand_data(data, soa->origin);
  mydns_check_name(soa, rr, name, data, cname_in, cnamelen_in, "rr.data", 1, 0);
  ...
  if (found)
    mydns_rrproblem(soa, rr, name, data, _("non-CNAME record(s) present alongside CNAME"));
}
/*--- __mydns_check_rr_cname() --------------------------------------------------------------------------*/
...
void __mydns_check_rr_hinfo(MYDNS_SOAP soa, MYDNS_RRP rr, const char *name, char *data,
  ...
  if (hinfo_parse((char*)hinfo_in, cpu, os, DNS_MAXNAMELEN) < 0)
    mydns_rrproblem(soa, rr, name, data, _("data too long in HINFO record"));
}
/*--- __mydns_check_rr_hinfo() --------------------------------------------------------------------------*/
...
void __mydns_check_rr_mx(MYDNS_SOAP soa, MYDNS_RRP rr, const char *name, char *data,
  ...
  mx_in = mydns_expand_data((char*)mx_in, soa->origin);
  mydns_check_name(soa, rr, name, data, mx_in, strlen(mx_in), "rr.data", 1, 0);
}
/*--- __mydns_check_rr_mx() --------------------------------------------------------------------------*/
...
void __mydns_check_rr_naptr(MYDNS_SOAP soa, MYDNS_RRP rr, const char *name, char *data,
  ...
  if (!strsep_quotes2(&p, &tmp))
    return mydns_rrproblem(soa, rr, name, data, _("'order' field missing from NAPTR record"));
  RELEASE(tmp);

  if (!strsep_quotes2(&p, &tmp))
    return mydns_rrproblem(soa, rr, name, data, _("'preference' field missing from NAPTR record"));
  RELEASE(tmp);

  if (!strsep_quotes2(&p, &tmp))
    return mydns_rrproblem(soa, rr, name, data, _("'flags' field missing from NAPTR record"));
  RELEASE(tmp);

  if (!strsep_quotes2(&p, &tmp))
    return mydns_rrproblem(soa, rr, name, data, _("'service' field missing from NAPTR record"));
  RELEASE(tmp);

  if (!strsep_quotes2(&p, &tmp))
    return mydns_rrproblem(soa, rr, name, data, _("'regexp' field missing from NAPTR record"));
  RELEASE(tmp);

  if (!strsep_quotes2(&p, &tmp))
    return mydns_rrproblem(soa, rr, name, data, _("'replacement' field missing from NAPTR record"));
  RELEASE(tmp);

  /* For now, don't check 'replacement'.. the example in the RFC even contains illegal chars */
  /* data = mydns_expand_data(tmp, soa->origin); */
  /* mydns_check_name(tmp, "replacement", 1, 0); */
  RELEASE(data_copy);
}
/*--- __mydns_check_rr_naptr() --------------------------------------------------------------------------*/
...
void __mydns_check_rr_ns(MYDNS_SOAP soa, MYDNS_RRP rr, const char *name, char *data,
  ...
  ns_in = mydns_expand_data((char*)ns_in, soa->origin);
  mydns_check_name(soa, rr, name, data, ns_in, strlen(ns_in), "rr.data", 1, 0);
}
/*--- __mydns_check_rr_mx() --------------------------------------------------------------------------*/
...
void __mydns_check_rr_rp(MYDNS_SOAP soa, MYDNS_RRP rr, const char *name, char *data,
  ...
  mydns_check_name(soa, rr, name, data, txt, strlen(txt), "rr.data (txt)", 1, 0);
  RELEASE(txt);
}
/*--- __mydns_check_rr_rp() --------------------------------------------------------------------------*/
...
void __mydns_check_rr_txt(MYDNS_SOAP soa, MYDNS_RRP rr, const char *name, char *data,
  ...
  if (txtlen_in > DNS_MAXTXTLEN) {
    return mydns_rrproblem(soa, rr, name, data, _("Text record length is too great"));
  }
  ...
  while (txtlen_in > 0) {
    ...
    if (len > DNS_MAXTXTELEMLEN)
      return mydns_rrproblem(soa, rr, name, data, _("Text element in TXT record is too long"));
    txt_in = &(txt_in[len]);
    txtlen_in -= len;
  }
}
